...
- The GitHub release includes the complete DAC runtime package, including mode0 state scheduling used by the submission, while excluding paper data, experiment result directories, Nsight artifacts, caches, and temporary diagnostics.
...
Synchronize the complete DAC runtime package plus packaging metadata:
...
Use `Copy-Item -LiteralPath` for the mechanical copy, excluding `__pycache__` and generated bytecode. Do not copy `docs/paper_*`, `experiments/`, result JSON, shell launchers, or caches.
...
- Source baseline: DAC submission runtime, research commit `38f20e3` plus the audited runtime working-tree diff recorded in this release commit.
...
- Excluded: paper experiments/results, motivation/evaluation scripts, profiling artifacts.
